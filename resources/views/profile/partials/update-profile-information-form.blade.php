<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            <x-form.input :label="__('Name')" :errors="$errors->get('name')" id="name" name="name" type="text"
                class="mt-1 block w-full" :value="old('name', $user->name)" required autofocus autocomplete="name" />
        </div>

        <div>
            <x-form.input :label="__('Email')" :errors="$errors->get('email')" id="email" name="email" type="email"
